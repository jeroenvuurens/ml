...
class trainer_assist:
    def __init__(self, trainer):
        ...
        self.optimizer = trainer.get_optimizer()
...
class ExponentialLR(_LRScheduler):
    ...
    def get_lr(self):
        curr_iter = self.last_epoch + 1
        r = curr_iter / self.num_iter
        return [base_lr / 100 * (self.end_lr / (base_lr / 100)) ** r for base_lr in self.base_lrs]

def sweep_lr(start_lr, end_lr, batches):
    def generate():
        current += 1
        yield start_lr * (end_lr / start_lr) ** (current / batches)
...
class LRFinder(trainer_assist):
    ...
    def range_test(
        ...
        """
        train_loader = self.trainer.get_databunch().train_dl
        val_loader = self.trainer.get_databunch().valid_dl
